...
Function webDriver { 
    ...
    Elseif((($gpu.Supported -eq "unOfficial") -and ($gpu.cloudprovider -eq "aws") -and ($gpu.Device_ID -eq "DEV_1EB8")) -eq $true){
        ...
        $G4WebDriver.tostring().split('/')[2].split('_')[0]
        }
...
function DownloadDriver {
    if((($gpu.Supported -eq "UnOfficial") -and ($gpu.cloudprovider -eq "aws") -and ($gpu.Device_ID -eq "DEV_1EB8")) -eq $true){
        ...
        Rename-Item -Path ($($system.Path) + "\NVIDIA_" + $($gpu.web_driver) + ".exe") -NewName "NVIDIA_$($gpu.web_driver).exe"
        Move-Item -Path ($($system.Path) + "\NVIDIA_" + $($gpu.web_driver) + ".exe") -Destination $system.Path
